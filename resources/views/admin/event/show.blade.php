...
@section('content')
    <div class="container-fluid">
        ...
        <a href="{{ route('admin.event.index') }}" class="btn btn-danger waves-effect">BACK</a>
        @if($event->is_approved == false)
            <button type="button" class="btn btn-success waves-effect pull-right" onclick="approveEvent({{ $event->id }})">
                <i class="material-icons">done</i>
                <span>Approve</span>
            </button>
            <form method="post" action="{{ route('admin.event.approve',$event->id) }}" id="approval-form" style="display: none">
                @csrf
                @method('PUT')
            </form>
        @else
